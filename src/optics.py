...
def generate_psfs_multi(height_maps, oof_phase, wavelengths, aperture_index,
                       size_r, size_g, size_b):
    """
    为 RGB 三个通道分别使用独立的高度图生成 PSF。
    
    Args:
        height_maps: 形状为 [3, N_B, N_B] 的张量，分别对应 R, G, B 的高度分布
        oof_phase: 离焦相位 [N_Phi, N_B, N_B, 3]
        wavelengths: 波长数组 [3]
        aperture_index: 孔径掩膜 [N_B, N_B]
    """
    n = 1.5  # 折射率
    kernel_size = size_b
    ...
    # 转换为 tensor
    oof_phase = torch.from_numpy(oof_phase).to(device) if isinstance(oof_phase, np.ndarray) else oof_phase
    aperture_index = torch.from_numpy(aperture_index).to(device) if isinstance(aperture_index, np.ndarray) else aperture_index

    # --- 分通道计算逻辑 ---
    
    # 1. Red Channel
    phase_r = 2 * np.pi / wavelengths[0] * (n - 1) * height_maps[0].unsqueeze(0) + oof_phase[:, :, :, 0]
    pupil_r = aperture_index.unsqueeze(0) * torch.exp(1j * phase_r)
    # 补零与 FFT (逻辑同 generate_psfs)
    pad_r = (size_r - kernel_size) // 2
    pupil_r = F.pad(pupil_r, (pad_r, pad_r, pad_r, pad_r))
    psf_r = torch.abs(torch.fft.fftshift(torch.fft.fft2(pupil_r), dim=(-2, -1)))**2
    psf_r = psf_r[:, pad_r:-pad_r, pad_r:-pad_r] if pad_r > 0 else psf_r

    # 2. Green Channel
    phase_g = 2 * np.pi / wavelengths[1] * (n - 1) * height_maps[1].unsqueeze(0) + oof_phase[:, :, :, 1]
    pupil_g = aperture_index.unsqueeze(0) * torch.exp(1j * phase_g)
    pad_g = (size_g - kernel_size) // 2
    pupil_g = F.pad(pupil_g, (pad_g, pad_g, pad_g, pad_g))
    psf_g = torch.abs(torch.fft.fftshift(torch.fft.fft2(pupil_g), dim=(-2, -1)))**2
    psf_g = psf_g[:, pad_g:-pad_g, pad_g:-pad_g] if pad_g > 0 else psf_g

    # 3. Blue Channel
    phase_b = 2 * np.pi / wavelengths[2] * (n - 1) * height_maps[2].unsqueeze(0) + oof_phase[:, :, :, 2]
    pupil_b = aperture_index.unsqueeze(0) * torch.exp(1j * phase_b)
    psf_b = torch.abs(torch.fft.fftshift(torch.fft.fft2(pupil_b), dim=(-2, -1)))**2

    # 归一化并堆叠
    norm = kernel_size * kernel_size * torch.sum(aperture_index**2)
    psfs = torch.stack([psf_r, psf_g, psf_b], dim=-1) / norm
...
def create_height_map_from_zernike(zernike_basis, num_modes, kernel_size,
    ...
    # Linear combination of Zernike modes
    g = torch.matmul(torch.from_numpy(zernike_basis).float(), zernike_coeffs).t()

    # Height map should be all positive
    height_map = F.relu(g.view(3, kernel_size, kernel_size) + wavelengths.view(3, 1, 1))
