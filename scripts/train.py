...
def train():
    ...
    # Fisher Mask 初始化
    if os.path.exists(config.FISHER_MASK_PATH):
        print("Initializing with Fisher Mask...")
        fisher_height = np.loadtxt(config.FISHER_MASK_PATH)
        # 调用拟合函数 (确保该函数定义在 train.py 上方)
        initial_coeffs = optics.fit_zernike_to_height_map(fisher_height, zernike_basis)
        # 将拟合得到的系数强制写入模型的参数中
        with torch.no_grad():
            model.zernike_coeffs.copy_(initial_coeffs.to(device))
        print("Successfully initialized with Fisher Mask!")
    else:
        print("Fisher Mask file not found, starting from zeros.")
        ...
        for batch_idx, (rgb_batch, dpphi_batch, phi_batch, mask_batch) in enumerate(train_loader):
            ...
            if global_step % config.SAVE_INTERVAL == 0:
                ...
                with torch.no_grad():
                    ...
                    # Get height map and PSFs
                    height_map = model.get_height_map().cpu().numpy()
